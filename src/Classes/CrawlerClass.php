<?php 
/**
 * This file facilitates the crawler-specific functionality of the application.
 *
 * PHP version 7
 * 
 * @category Mycategory
...
/**
 * Define the CrawlerClass and set its functionality.
 *
 * PHP version 7
 * 
 * @category Mycategory
...
{
    /**
     * Initialize the crawl method and set its properties.
     *
     * @param string $url 
     * 
     * @return array
        ...
        if ($url != '') {
            ...
            foreach ($allUrls as $url) {
                ...
                if (strpos($url, '#') === false) {
                    ...
                    if (strpos($url, 'https://'.$parse['host']) !== false  
                        || strpos($url, 'https://www.'.$parse['host']) !== false
                    ) {
                        //Replacing the / at the end of any url if present
                        if (substr($url, -1, 1) == '/') {
                            $url = substr_replace($url, "", -1);
                    ...
                    if (strpos($url, $parse['host']) === false) {
                        if (substr($url, 0, 1) == '/') {
                            ...
                            if ($url != $parse['host']) {
                                /*Checking if the link is already preset in the 
                                final array and the common array*/
                                if (!in_array($secondUrl, $urls)  
                                    && !in_array($secondUrl, $allUrls)  
                                    && !in_array($newUrl, $allUrls)
                                ) {
                                    if ($prefix == 'https') {
                                        $newUrl = $secondUrl;
                                    }
